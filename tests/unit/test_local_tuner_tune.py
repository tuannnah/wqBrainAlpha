...
import src.operators_local  # noqa: F401  # đăng ký operator thật vào REGISTRY (parse ts_delta…)
...
def test_tune_chua_budget_cho_config_du_expr_nhieu_hang():
    # expr nhiều hằng window -> Giai đoạn 1 dễ nuốt hết budget nhỏ; tune phải CHỪA đủ chỗ
    # cho Giai đoạn 2 (config). truncation=0.02 cho điểm cao hơn -> phải được chọn dù expr
    # nhiều hằng và budget nhỏ. (Trước fix: Phase 1 ăn hết budget=8 -> config không được quét.)
    expr = ("add(ts_mean(close, 5), add(ts_mean(close, 5), "
            "add(ts_delta(close, 5), add(ts_delta(close, 5), ts_delta(close, 5)))))")

    def eval_fn(node, config):
        return 2.0 if abs(config.truncation - 0.02) < 1e-9 else 0.5

    res = tune(expr, _cfg(), data=None, budget=8, eval_fn=eval_fn)
