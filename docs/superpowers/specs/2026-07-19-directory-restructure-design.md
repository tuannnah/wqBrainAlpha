...
### Phát hiện khi rà kỹ: helper dùng chéo nhiều nhóm

Rà từng lời gọi hàm trong `main.py` cho thấy một số helper KHÔNG thuộc riêng 1 domain mà
được nhiều nhóm lệnh cùng dùng: `_make_client` (dùng bởi auth/fields/simulate/closed_loop/
research/marathon/submit/menu), `_cached_symbols`+`_local_operator_arity`+
`_portfolio_config_from_opts` (dùng bởi generate/closed_loop/research/marathon/llm/menu ở
các mức khác nhau), `_make_validated_simulator` (dùng bởi research). Ngoài ra một số nhóm
gọi thẳng sang nhóm khác: `closed_loop` và `marathon` đều gọi `_make_research_loop` (thuộc
research); `marathon` gọi `resolve_direction` (thuộc research) và `_make_llm_generator`
(thuộc llm); `menu` gọi `_make_router` (thuộc llm) và `_run_closed_loop_session` (thuộc
closed_loop). Đồ thị phụ thuộc này **không có vòng lặp** (DAG), nên tách được an toàn nếu
làm đúng thứ tự, nhưng cần thêm 1 module `common.py` cho helper dùng chung + vài import chéo
tường minh giữa các module — bảng dưới đây đã cập nhật để phản ánh đúng thực tế (thay cho
bảng dự kiến ban đầu lúc brainstorm).

### Bảng ánh xạ module (theo dòng hiện tại trong `main.py`) — ĐÃ CẬP NHẬT

| Module đích | Nội dung chuyển sang | Cần import chéo thêm |
|---|---|---|
| `src/app/cli/common.py` | `_make_client` (65), `_cached_symbols` (404), `_local_operator_arity` (436), `_make_invalid_field_recorder` (450), `_make_validated_simulator` (461), `_portfolio_config_from_opts` (479) | — |
| `src/app/cli/auth.py` | `prompt_credentials` (52), `login` (75) | `_make_client` từ common |
| `src/app/cli/fields.py` | `probe_fields` (110), `warm_cache_cmd` (135), `fetch_fields` (177), `cache_status` (213), `fetch_operators` (235), `list_fields` (256) | `_make_client` từ common |
| `src/app/cli/simulate.py` | `simulate` (299), `sweep_config` (341) | `_make_client` từ common |
| `src/app/cli/generate.py` | `generate` (499), `score_one_cmd` (575) | `_cached_symbols`, `_portfolio_config_from_opts` từ common |
| `src/app/cli/submit.py` | `submit` (1584) | `_make_client` từ common |
...
| `src/app/cli/llm.py` | `_make_deepseek` (931), `run_deepseek_smoke` (954), `describe_deepseek_smoke_error` (981), `check_deepseek` (993), `_make_router` (1024), `_make_llm_generator` (1036), `llm_generate` (1464), `llm_ideas` (1494) | `_cached_symbols`, `_local_operator_arity` từ common |
| `src/app/cli/research.py` | `_make_pool_corr_fn` (1051), `_make_pnl_fn` (1069), `_make_research_loop` (1104), `_render_research_result` (1192), `resolve_direction` (1236), `research` (1249), `_run_research_with_progress` (1321) | `_cached_symbols`, `_local_operator_arity`, `_make_validated_simulator`, `_make_client` từ common; `_make_router`, `_make_llm_generator` từ llm |
| `src/app/cli/closed_loop.py` | `_resolve_base_seed` (626), `_run_reseed_until_quota` (635), `_local_neutralization` (652), `_closed_loop_configs` (668), `_run_closed_loop_session` (685), `closed_loop_cmd` (858) | `_cached_symbols`, `_portfolio_config_from_opts`, `_make_client` từ common; `_make_research_loop` từ research |
| `src/app/cli/marathon.py` | `_render_marathon_report` (1352), `_marathon_direction_provider` (1369), `_marathon_on_event` (1388), `_run_marathon_session` (1402), `marathon` (1435) | `_cached_symbols`, `_local_operator_arity`, `_make_client` từ common; `_make_llm_generator` từ llm; `resolve_direction`, `_make_research_loop` từ research |
| `src/app/menu.py` | `_MenuState` (1692), `_menu_counts` (1712), `_menu_login` (1723), `_menu_fields` (1755), `_menu_operators` (1771), `_find_market_data_dir` (1784), `_menu_test_engine` (1795), `_menu_auto_sim` (1893), `_menu_view_submit` (1916), `_print_menu` (1962), `start` (2003) | `_cached_symbols`, `_local_operator_arity`, `_portfolio_config_from_opts`, `_make_client` từ common; `_make_router` từ llm; `_run_closed_loop_session` từ closed_loop |
| `main.py` (giữ lại) | `_setup_logging` (41), khởi tạo `app = typer.Typer()`, import + gắn command từ từng module con | — |

**Thứ tự tách bắt buộc theo DAG phụ thuộc** (để module sau import được module trước):
`common` → (`auth`, `fields`, `simulate`, `generate`, `submit`, `report`, `migrate`, `llm` —
thứ tự tự do trong nhóm này) → `research` → (`closed_loop`, `marathon` — thứ tự tự do) →
`menu`.

Mỗi module CLI con export các Typer command function dạng hàm trần (không decorator);
`main.py` import và gắn bằng `app.command(name)(fn)` với 1 instance `app` duy nhất định
nghĩa trong `main.py` (tránh sub-app Typer phức tạp không cần thiết cho quy mô này).
